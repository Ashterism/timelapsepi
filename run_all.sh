...
if [ "$WIFI_MODE" == "client" ]; then
  if who | grep -q "pts"; then # SSH session detected – skip to avoid dropping connection
    echo "[INFO] SSH session detected - skipping Wi-Fi operations" >> "$LOG_PATH"
  else # No SSH session – safe to bring up wlan0 in client mode
    ...
    if ping -c 1 -W 5 8.8.8.8 > /dev/null 2>&1; then
      ...
      if who | grep -q "pts"; then
        echo "[INFO] SSH session detected – skipping Wi-Fi reset" >> "$LOG_PATH"
      else
    ...
    # DROP WIFI?  Drop Wi-Fi after tasks to save battery (if no active SSH session)
    if [ "$DROP_WIFI_AFTER" == "True" ]; then
      if who | grep -q "pts"; then
        echo "[INFO] SSH session detected - keeping Wi-Fi on" >> "$LOG_PATH"
      else
        echo "[INFO] Dropping Wi-Fi after tasks to conserve power" >> "$LOG_PATH"
  ...
  echo "[INFO] Wi-Fi mode: none - no connectivity mode active (silent logging only)" >> "$LOG_PATH"
else
  echo "[WARN] Unknown Wi-Fi mode: $WIFI_MODE - skipping all network activity" >> "$LOG_PATH"
fi
